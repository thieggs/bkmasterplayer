...
/// O ganho da agulha também sobe e desce em rampa: cortar o som de uma vez,
/// ao encostar na parede da memória ou ao passar do limite, é um clique.
const VINYL_GAIN_RAMP_SECS: f32 = 0.004;
...
struct Vinyl {
    ...
    max: f32,
    /// Ganho da agulha agora, e o passo da rampa dele.
    gain: f32,
    gain_step: f32,
...
impl Vinyl {
    fn new(rate: u32) -> Self {
        Self {
            ...
            max: f32::INFINITY,
            gain: 1.0,
            gain_step: 1.0 / (rate as f32 * VINYL_GAIN_RAMP_SECS),
            tmp: vec![0.0; BLOCK * 2],
            active: false,
        }
    }
    ...
    fn render(&mut self, slot: &mut Slot, out: &mut [f32], n: usize) {
        ...
        for i in 0..n {
            ...
            // Acabou o que dava para decodificar: a agulha encosta na beirada
...
mod vinyl_tests {
    ...
    /// Anel com espaço para `cap` frames, mas só `fill` já decodificados: o
    /// resto chega depois, como a produtora do deck faria.
    fn ramp_slot_cap(cap: usize, fill: usize) -> (rtrb::Producer<f32>, Slot) {
        let (mut p, c) = rtrb::RingBuffer::<f32>::new(cap * 2);
        feed(&mut p, 0, fill);
    ...
    /// A agulha voltou a tocar? (com a rampa de ganho, o som volta alguns
    /// frames depois do motivo dele.)
    fn voltou(out: &[f32]) -> bool {
        out[out.len() - 200..].iter().any(|x| *x != 0.0)
    }

    /// Reprodução normal, com ninguém no disco: é assim que a memória enche.
    ...
    /// A agulha calou de vez? (o ganho desce em rampa, não de uma vez: o
    /// silêncio chega alguns frames depois do motivo dele.)
    fn calou(out: &[f32]) -> bool {
        out[out.len() - 200..].iter().all(|x| *x == 0.0)
    }
    ...
    fn disco_parado_e_giro_rapido_demais_nao_fazem_som() {
        ...
        v.target = 0.0;
        v.speed = 0.0;
        assert!(calou(&play(&mut v, &mut slot, 500)), "disco parado fez som");
        v.max = 4.0;
        v.target = v.max;
        v.speed = v.max;
        assert!(calou(&play(&mut v, &mut slot, 500)), "giro rápido demais fez som");
    }
    ...
    fn com_limite_alto_ainda_ha_limite() {
        ...
        v.target = 12.0;
        v.speed = 12.0;
        assert!(calou(&play(&mut v, &mut slot, 500)));
    }
    ...
    fn na_parede_da_memoria_a_agulha_cala_em_vez_de_zumbir() {
        ...
        v.target = -1.0;
        v.speed = -1.0;
        let out = play(&mut v, &mut slot, 2000);
        assert!(out[0] != 0.0, "devia ter voltado tocando");
        // Encostou na parede e calou: segurar a mesma amostra seria um zumbido.
        assert!(calou(&out), "zumbiu na parede da memória");
    }

    // ---- Calar é sempre temporário: quando o motivo passa, a agulha volta
    // sozinha, sem o dedo ter que pedir de novo. ----
    ...
    fn chegando_mais_audio_a_agulha_volta_sozinha() {
        ...
        feed(&mut p, 10, 1000);
        assert!(voltou(&play(&mut v, &mut slot, 500)), "não voltou sozinha");
    }
    ...
    fn saindo_da_parede_da_memoria_a_agulha_volta_sozinha() {
        ...
        v.target = -1.0;
        v.speed = -1.0;
        assert!(calou(&play(&mut v, &mut slot, 2000)), "devia ter calado na parede");
        // Virou o disco para frente de novo.
        v.target = 1.0;
        v.speed = 1.0;
        assert!(voltou(&play(&mut v, &mut slot, 500)), "não voltou ao sair da parede");
    }
    ...
    fn abaixando_do_limite_a_agulha_volta_sozinha() {
        ...
        v.target = 8.0;
        v.speed = 8.0;
        assert!(calou(&play(&mut v, &mut slot, 500)), "devia ter calado acima do limite");
        v.target = 1.0;
        v.speed = 1.0;
        assert!(voltou(&play(&mut v, &mut slot, 500)), "não voltou abaixo do limite");
    }
