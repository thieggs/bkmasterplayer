#!/usr/bin/env bash
# Gera o pacote .deb do BK Music Player a partir do build release (instala em /opt/bkmusicplayer).
# Uso: ./dev/build_deb.sh            (depois de `cd app && flutter build linux --release`)
# Saída: dist/bkmusicplayer_<versão>_amd64.deb
set -euo pipefail
...
PKG="$ROOT/dist/pkg"
rm -rf "$PKG" && mkdir -p "$PKG/DEBIAN" "$PKG/opt/bkmusicplayer" "$PKG/usr/bin" \
  "$PKG/usr/share/applications" "$PKG/usr/share/icons/hicolor/512x512/apps" "$PKG/usr/share/icons/hicolor/256x256/apps"
cp -r "$BUNDLE/." "$PKG/opt/bkmusicplayer/"
ln -s /opt/bkmusicplayer/player_musica "$PKG/usr/bin/bkmusicplayer"
...
Exec=/opt/bkmusicplayer/player_musica
Icon=$APP_ID
Terminal=false
...
Description: BK Music Player - player de música com AutoMix DJ
...
OUT="$ROOT/dist/bkmusicplayer_${VERSION}_amd64.deb"
